...
export function PostPageClient({ slug, initialPost }: { slug: string; initialPost?: Post }) {
  const posts = usePosts();
  // Serverul a rezolvat deja articolul (din cod sau din Firestore). Îl folosim ca
  // valoare inițială, ca să nu existe un flash de „Articol inexistent" până când
  // sosește primul snapshot din browser. Store-ul preia dacă are ceva mai proaspăt.
    ...
    <>
      <div className="border-b border-mist bg-cloud">
        <nav className="mx-auto flex max-w-3xl items-center gap-1.5 px-4 py-3 text-sm text-navy-400 sm:px-6">
          <Link href="/" className="inline-flex min-h-11 items-center hover:text-navy-700">Acasă</Link>
          <ChevronRight className="h-4 w-4" />
          <Link href="/blog" className="inline-flex min-h-11 items-center hover:text-navy-700">Blog</Link>
          <ChevronRight className="h-4 w-4" />
